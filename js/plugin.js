...
(function () {
    ...
    var PLUGIN_VERSION = '1.6.1';   // keep in sync with manifest.json
    ...
    function describeFilters(f) {
        ...
        if (f.folder) bits.push(state.folderNames && state.folderNames[f.folder]
            ? state.folderNames[f.folder] : 'folder');
        return bits.join(' · ');
    }
    ...
    /* run an async op over an array with limited concurrency.
       NOTE: only min(limit, items.length) workers are started, so completion
       must be counted against THAT number — comparing against `limit` instead
       never settled whenever the array was shorter than the limit (the caller
       awaited a promise that could never resolve). */
    function mapLimit(items, limit, fn) {
        return new Promise(function (resolve, reject) {
            ...
            function worker() {
                if (failed) return;
                if (i >= items.length) { if (++done === workers) resolve(out); return; }
                var idx = i++;
                Promise.resolve()
    ...
    function populateFolders(folders) {
        ...
        var names = {};
        sorted.forEach(function (fo) { if (fo && fo.id) names[fo.id] = fo.name || fo.id; });
        state.folderNames = names;
        if (el.folderSel) {
            var cur = (state.lens.filters && state.lens.filters.folder) || '';
            el.folderSel.innerHTML = '<option value="">Folder: any</option>';
    ...
    async function doSimilar() {
        ...
        try {
            ...
            for (var i = 0; i < sel.length; i++) {
                ...
                if (!res || !Array.isArray(res.results)) { seedsFailed++; continue; }
            ...
            if (!merged.size) {
                var allFailed = seedsFailed >= sel.length;
                el.grid.innerHTML = '';
                el.grid.hidden = true;
                el.lensCount.innerHTML = '0 items';
                if (allFailed) {
                    // every AI Search call failed: report the failure, don't claim
                    // the library simply has no similar images
                    el.emptyHint.innerHTML = '<p><b>Similar search didn’t respond.</b></p>' +
                        '<p>Eagle’s <b>AI Search</b> plugin returned no answer for the selected item(s). ' +
                        'Make sure it is installed and has finished indexing this library, then try again.</p>';
                    recordQueryError('aiSearch', 'searchByItemId', new Error('all seed searches failed'));
                } else {
                ...
                toast(allFailed ? 'AI Search didn’t respond — try again once it has finished indexing.' :
                    'No similar images found.', true, 4000);
                return;
            }
    ...
    async function runLens() {
        ...
        // An exclude-only lens is meaningful: "everything except this tag".
        // Without this, shift-clicking the only active chip emptied the lens.
        ...
        try {
                ...
                el.emptyHint.innerHTML =
                    (state.lastQueryError
                        ...
                        : (idx
                            ...
                            : '<p><b>The library could not be loaded.</b></p>')) +
                    (idx
            ...
            el.lensCount.innerHTML = fmt(state.lensTotal) + ' items <small>' +
                (state._extCount > 0
                    ? 'incl. ' + fmt(Math.max(0, state.lensTotal - state._extCount)) + ' here · ' +
                      fmt(state._extCount) + ' from other libraries (browse-only)'
    ...
    async function loadMore() {
        ...
        // The hydration below can await (slow page query on a large library), so
        // remember which lens this page belongs to: if the user reshapes the lens
        // meanwhile, this page must be dropped instead of appended to the new grid.
        var token = state.runToken;
        var pageIds = ids;
        // Claim this page immediately: state.loaded only advances after the await
        // below, so without this a second "Show more" click while the first page
        // is still hydrating would re-hydrate and append the very same items.
        state.loaded = to;
        ...
        try {
            ...
            if (indexCache && indexCache.byId) {
                ...
                var results = await mapLimit(pages, 3, function (pageIds) {
                    return withTimeout(eagle.item.get({ ids: pageIds }), PAGE_QUERY_TIMEOUT, 'get(ids)')
                        .catch(function () { pageFailures++; return []; });
                });
                items = [];
                results.forEach(function (r) { if (Array.isArray(r)) items = items.concat(r); });
                // A timed-out page must not silently drop its items: the count
                // would still claim them while the grid quietly shows fewer.
                if (pageFailures) {
                    state.lastQueryError = state.lastQueryError || {
                        kind: 'items', label: pageFailures + ' of ' + pages.length + ' page(s)',
                        msg: 'get({ids}) timed out'
                    };
                    dbg('hydration: ' + pageFailures + '/' + pages.length + ' page(s) timed out');
                }
            }
            ...
            if (typeof pageFailures !== 'undefined' && pageFailures) {
                toast('Some items couldn’t be loaded (Eagle query timed out) — the grid is incomplete.', true, 4500);
            }
